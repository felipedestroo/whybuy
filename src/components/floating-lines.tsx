...
const FloatingLines: React.FC<FloatingLinesProps> = ({
  ...
  // Use dark theme as default during SSR to avoid hydration mismatch
  const isDark = mounted ? resolvedTheme === "dark" : true;

  // Auto-detect gradient colors based on theme if not explicitly provided
  const effectiveGradient = linesGradient ?? (isDark
    ? ["#00AC8C", "#005845", "#009776"]  // dark mode - teal/green
    : ["#FF5373", "#FFA7BA", "#FF6889"]  // light mode - pink/coral
  );

  // Auto-detect blend mode based on theme if not explicitly provided
  const effectiveBlendMode = mixBlendMode ?? (isDark ? "normal" : "difference");
